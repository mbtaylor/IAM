...
\newcommand{\todonote}[1]{{\color{gray}\em [#1]}}
...
it is accessed.
\todonote{is this true?  I don't understand browser-based authentication
...
\verb|standard_id| to convey the method of presenting these credentials, and
\verb|access_url| to convey the location at which they should be presented.
The response from this login page is an X.509 certificate \todonote{chain?}
...
{\tt application/x-pem-file} \todonote{should it?}.
...
{\footnotesize
\begin{verbatim}
% curl -D - --header  'Cookie: JSESSIONID=474DEF8CBB046D47294' https://gea.esac.esa.int/tap-server/tap/async -o /dev/null -s
HTTP/1.1 200 200
