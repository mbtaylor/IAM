...
% \begin{description}
% \item[Reactive]
%       In the most simple case, an unauthenticated client requiring a
%       resource simply requests it anonymously.  If the response contains
%       the resource, typically with an HTTP 200 Success (perhaps via some
%       3xx redirects), no authentication is attempted.
%       However, if the response is a 401 or 403 accompanied by a supported
%       challenge, the client examines the challenge and negotiates
...
%       In case of a 401 or 403 without any challenges the client supports,
...
% \item[Preemptive]
%       The Reactive mode described above will not seek authentication
%       as long as a request successfully returns a resource.
%       However, in some cases a service may make successful responses
%       to both authenticated and unauthenticated requests,
%       but with different responses in the two cases.
%       This may be the case for instance for a TAP service
%       that holds some public and some private tables.
%       In this situation a client may want to detect
%       (for instance to indicate to the user)
%       that authentication, while not mandatory, is possible,
%       to enable access to protected data.
%       Such a situation should be detected at an early stage of
%       interaction with the service rather than, for instance,
%       revealed only with the result of a potentially expensive query.
%\end{description}
